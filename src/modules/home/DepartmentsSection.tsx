    ...
    <section className="py-24 bg-gray-50 dark:bg-gray-950 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-20">
          ...
          >
            Access specialized study materials and resources for each engineering
            department
          </motion.p>
        </div>
        ...
        <div className="relative flex overflow-hidden w-full pb-12">
          {/* Gradient Edges */}
          <div className="absolute top-0 bottom-0 left-0 w-12 md:w-32 bg-gradient-to-r from-gray-50 to-transparent dark:from-gray-950 z-10 pointer-events-none" />
          <div className="absolute top-0 bottom-0 right-0 w-12 md:w-32 bg-gradient-to-l from-gray-50 to-transparent dark:from-gray-950 z-10 pointer-events-none" />
          ...
          >
            {[...DEPARTMENTS, ...DEPARTMENTS].map((dept, index) => {
              const stats = globalStats?.departmentStats?.[dept.id] || {
                students: 0,
                resources: 0,
                subjects: 0,
              };
              ...
              return (
                ...
                >
                  <motion.div
                    ...
                    className="card h-full p-6 flex flex-col bg-white dark:bg-gray-800 rounded-3xl shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-700 hover:border-indigo-500/30 transition-all duration-300"
                  >
                    <div
                      className={`w-full h-24 bg-gradient-to-br ${dept.color} rounded-2xl mb-6 flex items-center justify-center group-hover:scale-105 transition-transform duration-500 relative overflow-hidden shadow-lg`}
                    >
                      {/* Glossy overlay */}
                    ...
                    <h3 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white mb-2 group-hover:text-indigo-600 transition-colors">
                      {dept.name}
                    </h3>
                    ...
                    <div className="space-y-1.5 mb-6 flex-1">
                      {dept.subjects.slice(0, 2).map((subject) => (
                        <div
                          key={subject}
                          className="flex items-center text-[11px] md:text-xs text-gray-600 dark:text-gray-400 font-bold group-hover:text-indigo-500 transition-colors"
                        >
                          <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 mr-2 shadow-sm shadow-indigo-500/50 flex-shrink-0" />
                          <span className="truncate">{subject}</span>
                        </div>
                      ))}
                      {dept.subjects.length > 2 && (
                        <div className="text-[9px] text-indigo-600 dark:text-indigo-400 font-black uppercase tracking-widest ml-3.5 pt-1">
                    ...
                    <div className="grid grid-cols-3 gap-1 py-4 border-t border-b border-gray-50 dark:border-gray-700/50 mb-4 font-bold uppercase tracking-tight">
                      <div className="text-center">
                        ...
                        <div className="text-[7px] md:text-[8px] text-gray-500">
                          Students
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-center space-x-2 text-xs md:text-sm font-bold text-indigo-600 dark:text-indigo-400 group-hover:translate-x-1 transition-transform">
                      <span>Explore</span>
